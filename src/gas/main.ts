...
import { parseCommand } from './plan-command';
import { SheetPlanStore } from './sheet-store';
...
export function doPost(event: PostEvent): GoogleAppsScript.Content.TextOutput {
    try {
        ...
        const result = executeCommand(parseCommand(payload), new SheetPlanStore(openSheet()));
