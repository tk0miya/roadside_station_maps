...
import type { PlanStore } from '../gas/api';
import { ApiError, type ErrorCode } from '../gas/errors';
import type { PlanPatch } from '../gas/plan-command';
...
// In-memory PlanStore standing in for the spreadsheet. Row numbers follow the
// sheet's convention: row 1 is the header, so data starts at 2.
export interface MemoryPlanStore extends PlanStore {
    entries(): PlanEntry[];
}

export const createMemoryPlanStore = (initial: PlanEntry[] = []): MemoryPlanStore => {
    const rows: PlanEntry[] = [...initial];

    return {
        list: (): PlanRow[] => rows.map((entry, index) => ({ rowNumber: index + 2, entry })),
        updateRow: (rowNumber: number, patch: PlanPatch) => {
...
export interface GasRuntimeOptions {
    ...
    // Name of the sheet (tab) the stub spreadsheet holds. Point SHEET_NAME at
    // something else to exercise the "sheet not found" path.
    sheetName?: string;
