...
│   ├── main.ts              doPost エントリーポイント + トークン認証 + シート読み書き
│   ├── update-target.ts     name + pref による行の特定・リネーム衝突チェック（純粋関数）
...
コードを触るときの制約はひとつだけ: **Apps Script のグローバル（SpreadsheetApp 等）に触れてよいのは
`main.ts` のみ。** 他のモジュールはシートから読んだ値を受け取る純粋関数に保ち、配列リテラルで
ユニットテストする。`main.ts` のテストだけは GAS グローバルのスタブが要るので、そこは配線
（認証してからシートを開くこと・正しいセルに書くこと）の確認に絞る。シートの列スキーマは
フロントエンドと共通で `src/shared/plan-types.ts` にある。
